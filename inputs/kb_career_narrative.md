...
I returned to Inflective recently (2024–2025) for a consulting engagement focused on program delivery and change management for a healthcare technology client transitioning to new ERP systems. I also integrated marketing and UX analytics, connecting Google Analytics 4 to Looker Studio to build user engagement dashboards. By analyzing user behavior, I provided data-driven UX recommendations and established escalation pathways that significantly improved cross-team communication and stakeholder coordination.
...
**I left Inflective in May 2025.** I am not currently employed at Inflective or any other company. Since then, I've been focused full-time on independent AI/ML engineering work and actively seeking my next role.

In parallel with the Inflective engagement and continuing after it, I built a portfolio of GenAI applications that represent the convergence of my full background:

**Resume Graph Explorer** — a full-stack application that transforms resumes into SKOS-compliant knowledge graphs. I designed a hybrid vocabulary with four namespaces (SKOS, ESCO, schema.org, custom), built a three-phase normalization pipeline with ESCO-anchored entity merging, and implemented a narrative synthesizer that produces conservative and exploratory career stories from graph analysis. The system supports provider-agnostic LLM extraction (Claude, OpenAI, Ollama), RDF export in three formats, and SPARQL queries. Deployed with Flask on Railway and React on Vercel. This project started from a simple question: what would a resume look like if it were designed for machines to reason about, not just humans to skim? It became my deepest exploration of semantic standards and ontology design.

**Digital Twin** — a RAG-powered chatbot that represents me conversationally, grounded in my actual work and writing. Built with ChromaDB, OpenAI embeddings (text-embedding-3-small), and Gradio on HuggingFace. The interesting design challenge was building the knowledge base *for* retrieval — writing documents shaped to improve how the system answers questions, not just storing everything I had. Lives at twin.barbhs.com. I built this because I wanted my portfolio to be a conversation, not a document — something a hiring manager or collaborator could actually interact with and come away understanding how I think.

**Weaving Memories Into Graphs** — a memorial knowledge graph for my late father, Domingo Hidalgo. A 14-entity Neo4j schema with LLM-powered extraction from his biosketch, Wikidata enrichment for external context, temporal relationship patterns, and a React + Vite frontend served by a Flask REST API. Deployed at domingo-hidalgo.com. The most personal project in my portfolio and the most technically complete knowledge graph implementation. I built this because I needed a way to preserve and honor the connections in my father's life — and because I believe knowledge graphs can carry meaning that flat documents cannot.

**Concept Cartographer** — a Gradio app that extracts concepts and relationships from LLM conversations and visualizes them as knowledge graphs in real time. Optimized from a two-call to single structured JSON architecture, reducing token usage and latency significantly. I built this because I wanted to make the structure of a conversation visible as it unfolds — externalizing reasoning rather than leaving it locked inside the model.

**Poolula Platform** — a modular RAG system with a built-in evaluation harness. Inspired by an actual need to automate compliance tracking for a small, family-owned LLC. Local-first architecture combining structured data and document retrieval (ChromaDB) with tool-style routing. Provider-agnostic LLM backend (OpenAI, Anthropic), audit-friendly logging, ground-truth evaluation workflow. Built with FastAPI and SQLModel. This is where I developed my approach to RAG evaluation as a first-class design concern — the evaluation harness wasn't an afterthought, it was the whole point.

**ConvoScope** — a multi-LLM conversation platform deployed on AWS EC2 with custom domain, SSL, and monitoring. Supports OpenAI, Anthropic, and Google providers with conversation management, real-time streaming, and topic extraction. Includes an evaluation framework with A/B testing methodology that achieved a 15% quality improvement. Captures latency and token-cost metrics to guide prompt compression tradeoffs. This was my first real attempt at LLM programming — I started with OpenAI and expanded to multi-provider when I discovered with genuine amazement how different the responses could be to the same question depending on the model. That surprise drove the whole project.

**GraphRAG with Podcasts** — a knowledge graph and vector retrieval system for podcast Q&A. Multi-stage ingestion pipeline with checkpointing and idempotent re-runs. Entities modeled in Neo4j Aura with GraphRAG-style retrieval supporting citation-grounded traceability. I was inspired by my favorite podcast, Catalog and Cocktails — which just so happens to be about enterprise data management and is broadcast from Austin. I wanted to build something that let me query a body of knowledge I already loved consuming.

**ChronoScope** — a Streamlit application extracting timeline events from resumes and generating interactive Plotly timelines and TimelineJS-ready exports. Local-first, privacy-sensitive design with validation checks for consistency across runs. I was inspired by how readily the brain organizes events by time — I wanted to try my hand at Event Extraction as a data modeling problem. I started with resumes because they naturally contain dates, then added TimelineJS compatibility because visual learning is one of the things I find most satisfying to build for.

**Beehive Analytics** — a knowledge graph transforming 4+ years of backyard beehive inspection photos into queryable colony intelligence using Google Cloud Vision API, Neo4j, vector embeddings for semantic search, and weather API integration. This started from a very practical frustration: I had a kitchen sink of inspection photos stored in Google Photos but no easy way to answer "when did we do our last inspection?" — even though the answer was low-hanging fruit from a metadata perspective. The project blossomed from there into a full knowledge graph with weather context and semantic search.

**Fitness Analytics Dashboard** — an end-to-end ML pipeline classifying 14 years of personal exercise data using scikit-learn and XGBoost, with drift detection, model retraining workflows, and an interactive dashboard with explainable AI features at workouts.barbhs.com. I was inspired by the fact that getting a dog meant I went on more walks and jogs, but my tracking app couldn't tell the difference because I'd always forget to select the right activity before starting. So I built an ML classifier to do it for me, and then designed a frontend to show me the metrics I actually cared about.
