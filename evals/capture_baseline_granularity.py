...
SCORE_RUBRIC = """
Coherence Score (1-5) — what you are scoring:
  Fragmentation: are chunks cut mid-sentence or mid-thought?
  Coverage:      do the chunks together contain enough to answer?
  Noise:         are off-topic chunks mixed in, wasting context space?
  NOTE: chunk ORDER does not matter — score only the contents.

  5 = Everything needed is here; no fragments, no noise
  4 = Mostly complete; one small gap or one minor off-topic chunk
  3 = Answerable, but requires working around gaps, cuts, or noise
  2 = Significant noise or fragmentation; LLM would struggle
  1 = Wrong content or so fragmented it's unusable
"""

SCORE_RUBRIC_COMPACT = "  Score the contents, not the order — 5=complete+clean  4=mostly  3=workable  2=noisy/cut  1=unusable"
